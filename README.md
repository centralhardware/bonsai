...
Поля "Место рождения" и "Место смерти" умеют подсказывать полные названия мест через [Google Places API](https://developers.google.com/maps/documentation/places/web-service/place-autocomplete).

Чтобы включить подсказки, создайте API-ключ в [Google Cloud Console](https://console.cloud.google.com/) с доступом к *Places API (New)* и укажите его в конфигурации:
...
Ключ используется только на сервере: браузер обращается к самому Bonsai, а тот проксирует запрос в Google.
Если ключ не указан, поля работают как обычные текстовые - подсказки просто не появляются.
